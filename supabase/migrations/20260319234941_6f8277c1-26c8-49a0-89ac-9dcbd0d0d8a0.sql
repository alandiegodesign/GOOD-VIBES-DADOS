...
BEGIN
  ...
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'name', NEW.raw_user_meta_data->>'full_name', ''), COALESCE(NEW.raw_user_meta_data->>'user_type', 'cliente'), COALESCE(NEW.raw_user_meta_data->>'phone', ''), NULLIF(TRIM(COALESCE(NEW.raw_user_meta_data->>'cpf', '')), ''), COALESCE(NEW.email, ''), NEW.raw_user_meta_data->>'avatar_url')
...
RETURNS TABLE(user_id uuid, user_name text, user_email text) LANGUAGE sql STABLE SECURITY DEFINER SET search_path = 'public' AS $$
  SELECT p.user_id, p.name, u.email FROM public.profiles p JOIN auth.users u ON u.id = p.user_id WHERE u.email = p_identifier OR p.cpf = p_identifier LIMIT 1;
$$;
